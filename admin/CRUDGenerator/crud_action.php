...
require('CRUDGenerator.class.php');
...
if(file_exists($root.'model/generated')) Doctrine::loadModels($root.'model/generated');
else if($GLOBALS["debugMode"]) echo "No se ha encontrador el directorio 'model/generated'";

if(file_exists($root.'model')) Doctrine::loadModels($root.'model');
else if($GLOBALS["debugMode"]) echo "No se ha encontrador el directorio 'model'";

$tablas = $_REQUEST['tablas'];

$crud = new CRUDGenerator("../../binding.xml");

foreach($tablas as $tabla)
{
	$crud->generateCRUD($tabla,true);
}
