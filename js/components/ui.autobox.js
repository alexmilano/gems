      ...
      input
        ...
        .keypress(function(e){
          ...
          if($.data(document.body, "autoboxMode") && k < 32 && k != KEY.BS && k != KEY.DEL) return false;
			  ...
			  $("body").trigger("additem",input.val());
            if(input.val() && opt.filter.test(input.val())){ addBox(input, input.val(), opt.name); }	
            e.preventDefault();
          }
		  else if(k == KEY.BS)
		  {
			  if(input.val()=='') 
			  {
				  $("body").trigger("removeitem",input.val());
				  removeLastBox();
			  }
		  }
          else if(k == KEY.DEL || k > 32){ // more than ESC and RETURN and the like
